...
class AlphaVantageConnector:
    ...
    def get_forex_quote(self, from_currency: str, to_currency: str) -> Optional[RealForexQuote]:
        """
        Obtém cotação real de Forex
...
class PolygonConnector:
    ...
    def get_forex_quote(self, from_currency: str, to_currency: str) -> Optional[RealForexQuote]:
        """
        Obtém cotação real de Forex via Polygon
...
class RealForexOracle:
    ...
    def get_quote(self, pair: str) -> Optional[RealForexQuote]:
        """
        Obtém cotação real com fallback automático
        ...
        for provider_type, provider in self.providers:
            try:
                quote = provider.get_forex_quote(from_currency, to_currency)
                if quote:
                    # Cache resultado
    ...
    def get_multiple_quotes(self, pairs: List[str]) -> Dict[str, Optional[RealForexQuote]]:
        """
        ...
        results = {}
        for pair in pairs:
            results[pair] = self.get_quote(pair)
            time.sleep(1)  # Rate limiting entre pares
        return results
